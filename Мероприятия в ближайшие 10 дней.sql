...
where c.id in (142, 146, 132, 130, 153, 135, 145, 139, 166, 140, 167, 144, 147, 131,
               175)   #Здесь айди контекстов из LABS (!!!), можно добавить (вставить в скобки числа через запятую)
  ...
  and date(t.startDT) between CURDATE() and DATE_SUB(CURDATE(), interval -10 day) # начало мероприятий в интервале дат с сегодняшнего (автоматически подставляет текущую дату, когда делаете выгрузку) и + 10 дней
order by c.title