...
from labs.activity a
         inner join labs.activity_track atrack on a.id = atrack.activityID
         inner join (select * from labs.track tr where tr.id in (224,223,226)) track on atrack.trackID = track.id
...
where e.isDeleted = 0
  and a.isDeleted = 0
  and r.isDeleted = 0
group by track.title, track.id, a.id, a.title, a.description, a.short, a.createDT, theme.title, aa.activity_authors, ty.title, r.id, e.id, e.title, e.description, date(t.startDT), timediff(time(t.startDT), '-03:00:00'), ea.activity_authors, rec.users, ch.checkins, ref.user_feedback, if(tm.N_mat is not null, tm.N_mat, 0) +
                if(um.N_mat is not null, um.N_mat, 0), if(tm.approved is not null, tm.approved, 0) +
                if(um.approved is not null, um.approved, 0), tm.tm_NULL_VALUE + um.us_OF_NULL_VALUE, if(tm.N_mat is not null, tm.N_mat, 0) -
                (if(tm.approved is not null, tm.approved, 0) + tm.tm_NULL_VALUE) +
                if(um.N_mat is not null, um.N_mat, 0) -
                (um.us_OF_NULL_VALUE + if(um.approved is not null, um.approved, 0))
